...
export default function RegisterScreen({ navigation }: any) {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      navigation.navigate('OTP', { userId: res.data.data.userId });
    } catch (error: any) {
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <Animated.View style={[s.header, { transform: [{ translateY: headerSlide }], opacity: headerOpacity }]}>
            <View style={[s.badgePill, { backgroundColor: theme.bgBadge, borderColor: theme.borderBadge }]}>
              <Text style={[s.badgeText, { color: theme.textBadge }]}>📊 MR Registration</Text>
            </View>
            <Text style={[s.titleText, { color: theme.textTitle }]}>Create{'\n'}Account</Text>
            <Text style={[s.subtitleText, { color: theme.textSub }]}>Register as Medical Representative</Text>
          </Animated.View>

          {/* MR Badge */}
          <View style={[s.mrBadgeCard, { backgroundColor: theme.bgCard, borderColor: theme.borderBadge }]}>
            <Text style={s.mrBadgeIcon}>📊</Text>
            <View>
              <Text style={[s.mrBadgeTitle, { color: theme.textBadge }]}>Medical Representative</Text>
              <Text style={[s.mrBadgeSub, { color: theme.textSub }]}>Field Verified Access</Text>
            </View>
          </View>
            ...
            >
              <View style={s.ctaInner}>
                {loading ? <ActivityIndicator color="#fff" /> : <>
                  <Text style={s.ctaText}>Register as MR</Text>
...
const s = StyleSheet.create({
  ...
  mrBadgeCard: { flexDirection: 'row', alignItems: 'center', gap: 14, borderWidth: 1.5, borderRadius: 16, padding: 16, marginBottom: 20 },
  mrBadgeIcon: { fontSize: 32 },
  mrBadgeTitle: { fontSize: 15, fontWeight: '700' },
  mrBadgeSub: { fontSize: 12, marginTop: 2 },
